...
const buildAthleteRecommendations = (data: PaddleMotionData, user: User): RecommendationDraft[] => {
  ...
  if (hardCount >= 3) {
    drafts.push(createDraft(
      ...
      "Drei oder mehr harte Einheiten in fuenf Tagen sind ein klares Belastungssignal.",
  ...
  athletes.forEach((athlete) => {
    ...
    if (weekPlan.length === 0) {
      drafts.push(createDraft(
        ...
        "Coach-Ansicht: Wochenplanung enthaelt keine Einheit fär diesen Sportler.",
    ...
    if (hardCount >= 3) {
      drafts.push(createDraft(
        ...
        "Mehrere harte Einheiten in kurzer Zeit koennen die Qualität senken.",
