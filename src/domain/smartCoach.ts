...
const buildAthleteRecommendations = (data: PaddleMotionData, user: User): RecommendationDraft[] => {
  ...
  if (riskyGoal) {
    drafts.push(createDraft(
      ...
      `${riskyGoal.goal.title} ist bald fällig und liegt noch unter 60 Prozent Fortschritt.`,
      `Fällig am ${riskyGoal.goal.dueDate}, aktueller Fortschritt ${Math.round(riskyGoal.progress)}%.`,
      "Brich das Ziel in eine konkrete Trainingseinheit für diese Woche herunter.",
      "season_goal",
      riskyGoal.goal.id,
    ));
  }
  ...
  athletes.forEach((athlete) => {
    ...
    if (doneWithoutFeedback.length > 0) {
      drafts.push(createDraft(
        ...
        "training",
        "medium",
        `${athlete.name}: Rückmeldung offen`,
        "Erledigte Trainings ohne Feedback erschweren die Belastungssteuerung.",
        `${doneWithoutFeedback.length} erledigte Einheit(en) ohne Rückmeldung.`,
        "Bitte um kurze Rückmeldung zu Gefühl, Müdigkeit und Motivation.",
        "coach_athlete",
        athlete.id,
        athlete.id,
      ));
  ...
  data.coachGroups.forEach((group) => {
    ...
    if (groupWeekPlan.length === 0 && (isAdminRole(user.role) || group.clubId === user.profile.club || group.coachUserId === user.userId)) {
      drafts.push(createDraft(
        ...
        "Diese Gruppe hat für die aktuelle Woche noch keine geplante Einheit.",
