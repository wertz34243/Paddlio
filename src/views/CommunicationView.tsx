...
export function CommunicationView({ data, user, onDataChange }: CommunicationViewProps) {
  ...
  const newsView = (
    <section className="section-block">
      ...
      {isCoachLike || isAdmin ? <form className="entry-form" onSubmit={createPost}><div className="form-grid"><label>Titel<input name="title" required /></label><label>Kategorie<select name="category">{postCategories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Prioritaet<select name="priority">{priorities.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Zielgruppe<select name="targetType">{targetTypes.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Ablaufdatum<input name="expiresAt" type="datetime-local" /></label><label className="toggle-row">Wichtig<input name="isPinned" type="checkbox" /></label></div><label>Inhalt<textarea name="content" rows={4} required /></label><button className="save-button" type="submit">Beitrag erstellen</button></form> : null}
  ...
  const tasksView = (
    <section className="section-block">
      ...
      {isCoachLike || isAdmin ? <form className="entry-form" onSubmit={createTask}><div className="form-grid"><label>Titel<input name="title" required /></label><label>Typ<select name="taskType">{taskTypes.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Prioritaet<select name="priority">{priorities.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Faellig<input name="dueDate" type="date" /></label><label>Empfaenger<select name="assignedTo"><option value={user.userId}>Ich</option>{contacts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div><label>Beschreibung<textarea name="description" rows={3} /></label><button className="save-button" type="submit">Aufgabe erstellen</button></form> : null}
  ...
  const attendanceView = (
    <section className="section-block">
      ...
      }) : <p className="empty-state">Noch kein Training fuer Anwesenheit gefunden.</p>}</div>
    </section>
  );
  ...
  const filesView = (
    <section className="section-block">
      ...
      <p className="muted">Supabase Storage Bucket `paddlio-files` ist vorbereitet. Uploads laufen spaeter ueber separate Storage Policies; diese Version speichert stabile Metadaten ohne App-Absturz.</p>
