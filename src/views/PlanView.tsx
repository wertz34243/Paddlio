...
export function PlanView({
  ...
  return (
    <div className={`stack calendar-shell planning-shell ${isSingleWorkspaceMode ? "training-single-workspace" : ""} ${isTemplatesOnlyMode ? "training-templates-only" : ""} ${isJournalOnlyMode ? "training-journal-only" : ""}`}>
      {copyMessage ? <p className="auth-message success">{copyMessage} Die eingefügten Trainings erscheinen im zentralen Kalender.</p> : null}
            ...
            {visibleGroups.length > 0 ? visibleGroups.map((group) => {
              ...
              return (
                <article className="calendar-training-card" key={group.id}>
                  ...
                  <div className="card-actions">
                    <button type="button" onClick={onOpenOverview}>Im Kalender anzeigen</button>
