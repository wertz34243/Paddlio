...
export const subscribeToUserTrainings = (userId: string, onChange: RealtimeHandler): (() => void) => {
  ...
  const channel = client
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "training_journal_entries", filter: `athlete_id=eq.${userId}` }, onChange)
...
export const subscribeToGeneralCloudChanges = (onChange: RealtimeHandler): (() => void) => {
  ...
  const channel = client
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "training_journal_entries" }, onChange)
