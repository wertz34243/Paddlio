...
Paddlio nutzt ausschließlich Supabase Auth. Neue Nutzer können selbst ein Konto erstellen und erhalten immer die Standardrolle `Athlete`. Rollen wie `Coach`, `TeamAdmin` und `Admin` werden nicht bei Registrierung vergeben, sondern später durch einen berechtigten Admin im Adminbereich beziehungsweise direkt in Supabase.
...
Alternativ kann die SQL-Datei im Supabase SQL Editor ausgeführt werden.
