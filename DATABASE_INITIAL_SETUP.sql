...
begin
  ...
  if not exists (select 1 from pg_constraint where conname = 'club_requests_requested_by_fk') then
    alter table public.club_requests add constraint club_requests_requested_by_fk foreign key (requested_by) references public.profiles(id) on delete set null;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'club_requests_reviewed_by_fk') then
    alter table public.club_requests add constraint club_requests_reviewed_by_fk foreign key (reviewed_by) references public.profiles(id) on delete set null;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'trainer_requests_user_id_fk') then
    alter table public.trainer_requests add constraint trainer_requests_user_id_fk foreign key (user_id) references public.profiles(id) on delete cascade;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'trainer_requests_club_id_fk') then
    alter table public.trainer_requests add constraint trainer_requests_club_id_fk foreign key (club_id) references public.clubs(id) on delete set null;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'training_groups_club_id_fk') then
    alter table public.training_groups add constraint training_groups_club_id_fk foreign key (club_id) references public.clubs(id) on delete cascade;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'training_groups_coach_id_fk') then
    alter table public.training_groups add constraint training_groups_coach_id_fk foreign key (coach_id) references public.profiles(id) on delete set null;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'group_members_group_id_fk') then
    alter table public.group_members add constraint group_members_group_id_fk foreign key (group_id) references public.training_groups(id) on delete cascade;
  end if;

  if not exists (select 1 from pg_constraint where conname = 'group_members_athlete_id_fk') then
    alter table public.group_members add constraint group_members_athlete_id_fk foreign key (athlete_id) references public.profiles(id) on delete cascade;
  end if;
end;
$$;
