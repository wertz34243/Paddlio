-- Reconcile the 0019/0024 authorization baseline on drifted DEV databases.
-- This migration is additive and idempotent. It must be run before 0038 when
-- repairing a database where the earlier migrations were only partly applied.
...
where primary_role is null
   or (active_club_id is null and club_id is not null);
  ...
  select public.paddlio_is_admin_415()
    or exists (
      ...
      where g.id = target_group_id
        and (g.coach_id = auth.uid() or public.paddlio_can_manage_club_415(g.club_id))
    );
$$;
...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      and p.status = 'active'
      and (p.club_id = target_club_id or p.active_club_id = target_club_id)
...
as $$
  select public.paddlio_is_admin_415()
    ...
    or (item.club_id is not null and public.paddlio_user_has_club_role_0024(item.club_id, array['Coach','ClubAdmin','Admin']))
    or (item.assigned_group_id is not null and exists (
      select 1 from public.group_memberships gm
      where gm.group_id = item.assigned_group_id and gm.user_id = auth.uid() and gm.status = 'active'
    ))
    or (item.assigned_group_id is not null and exists (
...
as $$
  select public.paddlio_is_admin_415()
    ...
    or item.coach_id = auth.uid()
    or exists (
      select 1 from public.training_plan_items t
      where t.id = item.training_plan_item_id and public.paddlio_can_read_training_item_0024(t)
    );
$$;
    ...
    or exists (
      ...
      where t.id = item.training_plan_item_id
        and (
          ...
          or (t.club_id is not null and public.paddlio_user_has_club_role_0024(t.club_id, array['Coach','ClubAdmin','Admin']))
        )
    );
$$;
...
using (user_id = auth.uid() or public.paddlio_is_admin_415() or public.paddlio_can_manage_club_415(club_id));
...
for select to authenticated
using (user_id = auth.uid() or public.paddlio_is_admin_415() or public.paddlio_can_manage_group_415(group_id));
...
using (
  ...
  or exists (
    select 1 from public.group_memberships gm
    where gm.group_id = training_groups.id and gm.user_id = auth.uid() and gm.status = 'active'
  )
);
...
using (
  public.paddlio_is_admin_415()
  or public.paddlio_can_manage_club_415(club_id)
  or public.paddlio_can_manage_club_415(active_club_id)
)
with check (
  public.paddlio_is_admin_415()
  or public.paddlio_can_manage_club_415(club_id)
...
do $$
begin
  if to_regprocedure('public.paddlio_is_admin_415()') is null
     or to_regprocedure('public.paddlio_can_manage_club_415(uuid)') is null
     or to_regprocedure('public.paddlio_can_manage_group_415(uuid)') is null
     or to_regprocedure('public.paddlio_user_has_club_role_0024(uuid,text[])') is null then
    raise exception 'Paddlio DEV authorization baseline reconciliation is incomplete';
  end if;
end $$;
