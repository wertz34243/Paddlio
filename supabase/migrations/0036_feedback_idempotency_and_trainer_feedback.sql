-- Keep one feedback record per training, athlete and feedback role.
-- Adds trainer feedback without creating a parallel feedback table.
...
-- Old databases may have lost the original two-column unique constraint. Retain
-- the newest row only where the semantic feedback identity is identical.
with ranked as (
  select id,
         row_number() over (
...
-- Remove the broad legacy ALL policy from 0001. The operation-specific 0024
-- policies remain active and now use the hardened function above.
drop policy if exists "training_feedback_scope_write" on public.training_feedback;

comment on index public.training_feedback_identity_0036 is
  'Idempotency key: one athlete and one trainer feedback per athlete and training.';
